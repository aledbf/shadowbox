...

{
	...
	printf '# date\t%s\n' "$(date -Is)"
	printf '# reps\t%s\n' "$REPS"
	printf 'metric\tcpus\tvendor\tmedian\tn\tunit\n'
	sort "$raw" | awk -F'\t' '
...
med() { awk -F'\t' -v m="$1" -v c="$2" -v v="$3" \
	'$1==m && $2==c && $3==v {print $4}' "$4"; }
...
printf '\n%-42s %5s %12s %12s %8s %s\n' metric cpus PVM KVM 'PVM/KVM' 'vs baseline'
printf '%-42s %5s %12s %12s %8s %s\n' \
	'------------------------------------------' ----- ------------ ------------ -------- -----------
...
metrics=$(awk -F'\t' '$1 !~ /^#/ && $1!="metric" && ($6=="ns"||$6=="us"||$6=="s") {print $1}' \
	"$results" | sort -u)
for m in $metrics; do
	for n in $CPUS; do
		...
		if [ "$have_baseline" = 1 ]; then
			...
			if [ -n "$bp" ]; then
				...
				worse=$(awk -v now="$p" -v was="$bp" -v t="$THRESHOLD" \
					'BEGIN{ print (was+0>0 && (now-was)/was*100 > t) ? 1 : 0 }')
				if [ "$worse" = 1 ]; then
		...
		printf '%-42s %5s %12.1f %12s %8s %s\n' \
			"$m" "$n" "$p" "$(awk -v v="$k" 'BEGIN{ if (v=="") print "-"; else printf "%.1f", v }')" \
			"$ratio" "$delta"
	done
done
